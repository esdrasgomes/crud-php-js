...
if(!empty($pagina)){
    ...
    if(($result_usuarios) and ($result_usuarios->rowCount() != 0)){
        ...
        // Paginação - Somar a quantidade de registros que há no banco de dados
        ...
        for($pag_ant = $pagina - $max_links; $pag_ant <= $pagina - 1; $pag_ant++ ) {
            ...
            if ($pag_ant >= 1) {
                $dados .= "<li class='page-item'>
                            <a class='page-link' href='#' onclick='listarUsuarios($pag_ant)'>$pag_ant</a>
                            </li>";
            }
        }

        $dados .= "<li class='page-item active' aria-current='page'>";
        $dados .= "<span class='page-link' href='#'>$pagina</span>";
        $dados .= "</li>";
        ...
        for($pag_dep = $pagina + 1; $pag_dep <= $pagina + $max_links; $pag_dep++) {
            ...
            if ($pag_dep < $quantidade_pg) {
                $dados .= "<li class='page-item'>
                            <a class='page-link' href='#' onclick='listarUsuarios($pag_dep)'>$pag_dep</a>
                            </li>";
            }
        ...
        $dados .= "<li class='page-item'>
                        <a class='page-link' href='#' onclick='listarUsuarios($quantidade_pg)'>Última</a>
                    </li>";
        $dados .= "</ul>";
        $dados .= "</nav>";
